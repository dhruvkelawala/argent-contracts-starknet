...
@pytest.fixture
async def get_multicall(get_starknet):
...
@pytest.mark.asyncio
async def test_multicall(get_starknet, get_multicall):
    starknet = get_starknet
...
@pytest.mark.asyncio
async def test_multicall_timestamp(get_multicall):
    multicall = get_multicall
    response = await multicall.get_current_block_timestamp().call()
    print(response.result.block_timestamp)
    assert response
